	...
	<xsl:template name="getStyleLink">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:element name="link">
					<xsl:attribute name="rel">stylesheet</xsl:attribute>
					<xsl:attribute name="href"><xsl:value-of select="$baseRef"/>prettify.css</xsl:attribute>
					<xsl:attribute name="type">text/css</xsl:attribute>
					<xsl:attribute name="media">screen</xsl:attribute>
				</xsl:element>
				<xsl:element name="link">
...
else if( window.attachEvent ) // Microsoft
{
  window.attachEvent( 'onload', prettyPrint );
	...
	<xsl:template name="getLinks">
		...
		<xsl:param name="showPackageConstants" select="true()"/>
		<xsl:param name="showPackageProperties" select="true()"/>
		<xsl:param name="showPackageFunctions" select="true()"/>
		<xsl:param name="showInterfaces" select="true()"/>
		<xsl:param name="showClasses" select="false()"/>
		<xsl:param name="showPackageUse" select="true()"/>
  ...
  ga('send', 'pageview');
						...
						<div class="feedbackLink">
							<center>
								...
								<xsl:if test="$config/feedback[@type='livedocs']">
									<a href="https://github.com/zwetan/as3-universal-analytics/issues" target="_blank">
	...
	<xsl:template name="insertAVMIcon">
		<xsl:param name="baseRef"/>
		<img src="{$baseRef}images/AVMIcon12x12.png" width="12" height="12" hspace="0" vspace="0" alt="AVM-only" title="Only available in the AVM runtime"/>
	</xsl:template>
</xsl:stylesheet>
